...
interface UseStripeInvoice2PaymentProps {
  invoiceId: string;
  currency: string;
}

export function useStripeInvoice2Payment({ invoiceId, currency }: UseStripeInvoice2PaymentProps) {
  ...
  const createPaymentIntent = useCallback(
    async (amount: number, type: PaymentType) => {
      ...
      try {
        setIsLoading(true);
        const response = await newRequest.post(`/invoices2/${invoiceId}/payment-intent`, {
